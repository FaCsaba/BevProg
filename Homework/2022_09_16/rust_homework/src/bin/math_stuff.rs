fn masodfoku(a: f64, b: f64, c: f64) -> (f64, f64) {
    return (-b + (b.powi(2) - 4.0*a*c).sqrt(), -b - (b.powi(2) - 4.0*a*c).sqrt())
}
...
fn main() {
    ...
    println!("{}", weighted_average(&vec![WeightAve {weight: 25, value: 75}, WeightAve {weight: 30, value: 60}]));
    println!("{}", standard_deviation(&vec![9, 2, 5, 4, 12, 7, 8, 11, 9, 3, 7, 4, 12, 5, 4, 10, 9, 6, 9, 4]))
}